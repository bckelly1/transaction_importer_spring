...
import com.brian.transaction_importer_spring.institution.fidelity.FidelityPositionSummaryImporter;
...
public class PositionImporterService {

    private final FidelityPositionSummaryImporter fidelityPositionSummaryImporter;


    public void parsePositionSummary(final MailMessage[] mailMessages) {
        for (MailMessage mailMessage : mailMessages) {
            KnownInstitution knownInstitution = parseInstitution(mailMessage);
            if (knownInstitution == KnownInstitution.FIDELITY) {
                fidelityPositionSummaryImporter.handlePositionSummary(mailMessage.getHtml());
            } else {
                throw new IllegalStateException("No position summary handler for institution " + knownInstitution);
            }
        }
    }
